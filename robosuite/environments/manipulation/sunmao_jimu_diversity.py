...
class SunmaoJimu(SingleArmEnv):
    ...
    def staged_rewards(self):
        ...
        cubeA_in_right_pos = (distance_A_B + (1 - rotation_cos_abs)) < 0.002
    ...
    def _sunmao_apperance(self):
        ...
        init_x_range = [0.1, 0.15]
        init_y_range = [0.1, 0.15]
        ...
        # sample init rotation
        # init_rotation = np.random.uniform(low=0, high=math.pi/4)
        init_rotation = 0.0
        for layer, slot in reversed(layer_slot_dict.items()):
            ...
            if layer != layer_num:
                ...
                self.placement_initializer.samplers[f"physical_jimu_objects_slot_{slot}_{1}"].rotation = layer_rotation_dict[layer] + init_rotation
                ...
                self.placement_initializer.samplers[f"physical_jimu_objects_slot_{slot}_{2}"].rotation = layer_rotation_dict[layer] + init_rotation
            else:
                ...
                self.placement_initializer.samplers[f"virtual_jimu_objects_slot_{slot}_{1}"].rotation = layer_rotation_dict[layer] + init_rotation
        ...
        for idx, slot in enumerate(moved_objects):
            ...
            rotation = np.random.uniform(low=0, high=math.pi/4)
            self.placement_initializer.samplers[f"moved_jimu_objects_slot_{slot}"].rotation = 0 + rotation
