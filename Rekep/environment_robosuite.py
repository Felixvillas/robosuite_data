...
class ReKepOGEnv:
    def __init__(self, config, scene_file, verbose=False):
        ...
        task = "stack"
        variant = json.load(open(f"./robosuite_config/{task}.json"))
    ...
    def get_sdf_voxels(self, resolution, exclude_robot=True, exclude_obj_in_hand=True):
        ...
        """
        start = time.time()
        exclude_names = ["world", "table", "robot"] # include world or not?
        ...
        trimesh_objects = []
        for obj_body_id, obj_name in self.og_env.sim.model._body_id2name:
            if any([name in obj_name.lower() for name in exclude_names]):
                continue
            ...
            # }
            if "cube" in obj_name:
                trimesh_object = trimesh.load_mesh("./robosuite_asset/cube.obj")
            else:
                raise NotImplementedError
            # trimesh_object = trimesh.Trimesh(**kwargs)
            
            world_pose_w_scale = T.pose2mat(
                (
                    self.og_env.sim.data.body_xpos[obj_body_id], 
                    T.convert_quat(self.og_env.sim.data.body_xquat[obj_body_id], to="xyzw")
                )
            )
    ...
    def close_gripper(self):
        ...
        action = np.zeros(7)
        action[6:] = [0]  # gripper: float. 0. for closed, 1. for open.
        for _ in range(30):
            self._step(action)
    ...
    def open_gripper(self):
        ...
        action = np.zeros(7)
        action[6:] = [1]  # gripper: float. 0. for closed, 1. for open.
        for _ in range(30):
            self._step(action)
        self.last_og_gripper_action = 1.0
